...
<table>
    <tr>
        <td align=left>
            <font color=yellow><% out.write(game.getName()); %></font><font color=white>Current
            Turn: <% out.write(game.getCurrentTurn() + " " + game.getPhase()); %>.
        </td>
        <td align=right>
            Old turns:
            <select id="oldturns" name="oldturns">
                <% String[] turns = game.getTurns();
                    for (int i = turns.length - 1; i >= 0; i--) {
                        String link = JolAdminFactory.INSTANCE.getId(game.getName()) + "-" + turns[i]; %>
