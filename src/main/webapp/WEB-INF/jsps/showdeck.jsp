...
<c:forEach items="${vampires.keySet()}" var="card">
    ${vampires.get(card)} x <a href="javascript:openWin('${card.cardId}')">${card.name} (G${card.group})</a><br/>
</c:forEach>
...
<c:forEach items="${deck.keySet()}" var="type">
    ...
    <c:forEach items="${typeMap.keySet()}" var="card">
        ${typeMap.get(card)} x <a href="javascript:openWin('${card.cardId}')">${card.name}</a><br/>
    </c:forEach>
</c:forEach>