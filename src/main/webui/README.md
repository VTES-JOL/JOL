...
`.storybook/preview.tsx` wraps every story in a `QueryClientProvider` + `MemoryRouter` (most components need at least one) and imports the same `src/main.tsx` global stylesheets the real app does (fonts, theme tokens, `styles/tailwind.css`, `card-visuals.css`). A component that reads a react-query cache the backend would normally fill (e.g. `TopBar`'s `useNav()`) gets a story-local decorator that seeds the cache with `queryClient.setQueryData(...)` instead of mocking a fetch — see `TopBar.stories.tsx`.
...
- `src/components/` — **shared, cross-page UI**: `TopBar`, `TabBar`, `CountryFlag`, `DeckPreview`, `ChunkErrorBoundary`, `UpdateBanner`, `PageLoading`, `ReconnectingOverlay`, plus the dialog/toast hosts — each with a co-located `*.stories.tsx` (see Testing above). All on the Tailwind kit / jol-quarkus tokens.
  - `src/components/ui/` — the **Tailwind component kit** (jol-quarkus tokens, Preflight base reset): `Button`, `Input`, `Select`, `Textarea`, `Switch`, `Card`, `Badge`, `Panel`, `Modal`, `EmptyState`, `Spinner`, `SectionLabel`, `FormFeedback` (`FieldHint`/`InlineAlert`), `MasterDetailView`, `icons/`. **Every route is on the kit** — the Bootstrap→Tailwind migration is complete (no Bootstrap CSS/JS/icons loaded anywhere; `jt:` utility prefix dropped). `/game` (`pages/game/*`) keeps only the server-emitted class strings it must (`card-name`/`.icon` from ParserService, discipline/clan/path icon fonts) — see `styles/card-visuals.css`. `components/ui/Panel` vs `pages/game/GamePanel` (game-only chrome) is the one remaining name overlap.
...
- `src/styles/` — global CSS only: `fonts.css`, `theme.css` (tokens/typography), `tailwind.css` (Tailwind v4 entry: Preflight reset + `@theme` token bridge + utilities layer, all imported in `main.tsx`), `card-visuals.css` (the shared card-name/icon vocabulary). Component- and page-scoped CSS is co-located instead (`pages/GamePage.css`, `pages/game/Clan.css`). Dark mode is `body[data-bs-theme="dark"]` (set by `TopBar`, keyed on by `tailwind.css`'s `@custom-variant dark`).
