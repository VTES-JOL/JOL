...
- `src/api/` — `client.ts` (thin fetch wrapper used by everything), `types.ts` (hand-written mirrors of the Java response beans), `config.ts`, `apiBase.ts`, `useConnectivity.ts`
...
- `src/components/` — **shared, cross-page UI**: `TopBar`, modal/dialog/toast hosts, `ChunkErrorBoundary`, `UpdateBanner`, `PanelPlaceholder`, `SplitLayout` — each with a co-located `*.stories.tsx` (see Testing above). Bootstrap-styled.
  - `src/components/ui/` — a **parallel Tailwind component kit** (`jt:`-prefixed classes) ported from the earlier jol-quarkus prototype: `Button`, `Input`, `Badge`, `Panel`, `EmptyState`, `MasterDetailView`, `icons/`. Currently used only by `pages/deck/`. Treat it as the target for *new* kit-styled screens; `src/components/*` is the legacy Bootstrap set — migrate a component across on touch, don't add new Bootstrap primitives. Note the intentional name overlaps while both exist: `components/ui/EmptyState` (Tailwind) vs `components/PanelPlaceholder` (Bootstrap); `components/ui/Panel` (Tailwind) vs `pages/game/GamePanel` (game-only chrome).
- `src/hooks/` — small reusable hooks (card tooltips, dropdown/tooltip open-close state, submit guard)
