...
- `src/components/` — **shared, cross-page UI**: `TopBar`, `Modal` (+ dialog/toast hosts), `TabBar`, `CountryFlag`, `Card`, `ChunkErrorBoundary`, `UpdateBanner`, `PanelPlaceholder`, `SplitLayout` — each with a co-located `*.stories.tsx` (see Testing above). Bootstrap-styled.
