...
export function DeckPreview({ deck }: { deck: Deck }) {
  ...
  return (
    <div ref={ref}>
      <div className="jt:font-semibold jt:text-xs jt:text-ink-muted jt:mt-1">Crypt ({deck.crypt.count})</div>
      <CardList cards={deck.crypt.cards} />
      <div className="jt:font-semibold jt:text-xs jt:text-ink-muted jt:mt-2">Library ({deck.library.count})</div>
      {deck.library.cards.map((section) => (
        <div key={section.type}>
          <div className="jt:font-semibold jt:text-xs jt:text-ink-muted jt:mt-1">
