...
1. Browser calls DWR-generated JS (`/dwr/interface/DS.js`) which proxies to **`DeckserverRemote`**
2. `DeckserverRemote` delegates to **`JolAdmin`** (the main singleton orchestrator) or services directly
...
- **`net.deckserver.dwr`**
  - `DeckserverRemote` — DWR endpoint; compiled to `/dwr/interface/DS.js`; all JS↔Java calls go here
  - `bean/` — JSON response objects returned to the frontend
  - `creators/` — populate beans for each page/view (`GameCreator`, `LobbyCreator`, etc.)
...
- **`net.deckserver.rest`** — Jersey JAX-RS endpoints (`GameResource`, `PlayerResource`, `NotificationResource`, `SystemResource`); experimental, not fully replacing DWR yet
...
- DWR generates `dwr/interface/DS.js` at runtime from `DeckserverRemote`. Any new Java method exposed there must also be registered in `WEB-INF/dwr.xml` as a converter if it returns a complex type.
- Frontend JS lives in `src/main/webapp/js/` and consumes the DWR-generated `DS.js`.
