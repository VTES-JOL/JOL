...
<%     boolean show = false;
       ...
       for(int i = 0; i < game.TURN_PHASES.length; i++) {
           ...
           if(show) { %>
    <option value="<% out.write(game.TURN_PHASES[i]); %>"><% out.write(game.TURN_PHASES[i]);%></option>
<%             }
           } %>
</select>
</td></tr><tr><td>
...
<% for(int i = 0; i < players.length; i++) { %>
    <option value="<% out.write(players[i]); %>"><% out.write(players[i] + "(" + game.getPingTag(players[i]) + ")");%></option>
<%  } %>
</select>
</td></tr><tr><td>
<% if(active) { %>
