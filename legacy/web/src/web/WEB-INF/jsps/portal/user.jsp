...
<li><a href="<% out.write(params.getPrefix()); %>?page=deck&deck='<% out.write(decks[i]); %>'"><% out.write(decks[i]); %></a>
Delete:<a href="<% out.write(params.getPrefix()); %>?page=user&delete=yes&deck='<% out.write(decks[i]); %>'">yes</a>
</li>
<%          }
        } %>
</ul>
...
<%  String[] games = admin.getGames(player);
    if(games == null) games = new String[0];
    Collection v = new ArrayList();
    for(int i = 0; i < games.length; i++) {
        if(admin.isOpen(games[i])) { 
            v.add(games[i]);
        }
    } 
    if(v.length == null) { %>
...
<option value="<% out.write(game); %>"><% out.write(game); %></option>
<% } %>
</select>
<select name=regdeck>
<% for(int j = 0; j < decks.length; j++) { %>
<option value="<% out.write(decks[i]); %>"><% out.write(decks[i]); %></option>
<% } %>
</select>
